...
describe('Utils', () => {
	it('normalizes paths across helpers', () => {
		...
		expect(isGenericSubdomain('src\\generic-subdomains\\utils\\x.ts')).toBe(true);
		expect(isRootLevelSourceFile('src\\Server.ts')).toBe(true);
	});

	it('detects test files and integration domains', () => {
		expect(isTestFile('src/domains/users/use-cases/commands/a/execute.test.ts')).toBe(
			true,
		);
		expect(
		...
		expect(
			extractIntegrationTestDomainFromPath('tests/integration/endpoints/setup.ts'),
		).toBeNull();
	});
});
