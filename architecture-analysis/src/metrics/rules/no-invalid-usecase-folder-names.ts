...
export function checkNoInvalidUseCaseFolderNames(cloc: ClocResult): Violation[] {
	...
	return violations.sort((a, b) =>
		a.path === b.path ? a.folder.localeCompare(b.folder) : a.path.localeCompare(b.path),
	);
}

export function printNoInvalidUseCaseFolderNames(cloc: ClocResult): void {
	const violations = checkNoInvalidUseCaseFolderNames(cloc);
